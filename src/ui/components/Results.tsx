  ...
  const health =
    r.seeders || r.leechers ? (
      <Text>
        ...
        <Text color={COLOR.dim}>{` seeders ${ICON.dot} ${r.leechers} leechers`}</Text>
      </Text>
    ) : (
      <Text color={COLOR.dim}>unknown</Text>
    );
  return (
    <Box flexDirection="column">
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        {r.numFiles ? (
          <DetailRow label="Files" value={<Text color={COLOR.dim}>{String(r.numFiles)}</Text>} />
        ) : null}
        {date ? <DetailRow label="Added" value={<Text color={COLOR.dim}>{date}</Text>} /> : null}
        <DetailRow
          label="Hash"
          value={
      ...
      <Box marginTop={1}>
        ...
        <Text color={COLOR.dim}> back</Text>
      </Box>
    </Box>
  );
}
...
export function Results() {
  ...
  const status = () => {
    if (search.loading) {
      if (results.length > 0)
        return <Text color={COLOR.dim}>{`searching… ${search.done}/${search.total} sources${sortNote}`}</Text>;
      return (
    ...
    if (results.length === 0) {
      ...
      if (search.results.length > 0 && activeCat?.group)
        return <Text color={COLOR.dim}>{`No ${activeCat.label.toLowerCase()} results yet. Try another tab or a search.`}</Text>;
      return (
    ...
    return <Text color={COLOR.dim}>{`${head}${note}${sortNote}`}</Text>;
  };
