/**
 * Proves windskye's seeding actually transfers bytes (not just a UI label), and
...
async function main(): Promise<void> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "windskye-seedcheck-"));
  ...
  // drop it. This is what windskye now captures at download time and seeds from.
